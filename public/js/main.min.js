<<<<<<< HEAD
/*! test - v1.0.0 - 2015-06-01-17-53 */
function el(element){return document.querySelector(element)}function messageToCLient(parent,parentClass,message,childClass){el(".tempDialog")?el(".tempDialog").parentNode.removeChild(el(".tempDialog")):element=document.createElement("div"),element.className=" tempDialog ",element.className+=childClass,parent.appendChild(element),parent.classList.add(parentClass),element.innerHTML=message}function pasteString(e){e.preventDefault()}for(var cardInput=el(".card"),inputs=document.querySelectorAll(".form-control"),cvvInput=el(".cvv"),dateCard=el(".dateClientCard"),cvvValue,element,getFieldValue=function(input){var _val=input.value;return _val},deleteNaN=function(field){var _fieldValue=getFieldValue(field),_notANumber=isNaN(_fieldValue);_notANumber&&(_fieldValue=_fieldValue.substring(0,_fieldValue.length-1)),field.value=_fieldValue},cardType=function(input){console.log(input.value);var _cardType,_val=getFieldValue(input),_firstnumber=_val.charAt(0),_secondNumber=_val.charAt(1);return 4==_firstnumber&&16==_val.length?(_cardType="Visa",cvvValue=0):5==_firstnumber&&16==_val.length?(_cardType="Mastercard",cvvValue=0):3!=_firstnumber||4!=_secondNumber&&7!=_secondNumber||15!=_val.length?_cardType="":(_cardType="American Express",cvvValue=1),_cardType},printCardType=function(resultCardType){var _buttonName=el(".input-group-addon");resultCardType?(_buttonName.classList.add("valid"),_buttonName.innerHTML=resultCardType):(_buttonName.classList.remove("valid"),_buttonName.innerHTML="")},isCardNumberValid=function(cardNo){var validateCC=function(arr){return function(ccNum){for(var val,len=ccNum.length,bit=1,sum=0;len;)val=parseInt(ccNum.charAt(--len),10),sum+=(bit^=1)?arr[val]:val;return sum&&sum%10===0}}([0,2,4,6,8,1,3,5,7,9]);return validateCC(cardNo)},isValidDate=function(){var now=new Date,month=now.getMonth()+1,year=now.getFullYear(),limitYear=now.getFullYear()+4;month=month.toString(),year=year.toString().substring(2,4),limitYear=limitYear.toString().substring(2,4);var _dateValue=dateCard.value.replace(/\s/g,""),_cardValidity=_dateValue.split(/\D/),_cardMonth=parseInt(_cardValidity[0]),_cardYear=_cardValidity[1],parent=dateCard.parentElement,parentClass="addtext";if(dateCard.value=_dateValue,""==_cardValidity[0])console.log("mmh");else if(2!=_cardValidity.length||_cardMonth>12||year>_cardYear||month>_cardMonth&&_cardYear==year||_cardYear>limitYear){var message="Write the date as on your card please",childClass="errorMessage";dateCard.classList.add("redBorder"),messageToCLient(parent,parentClass,message,childClass)}else{var message="Seems Good dawg",childClass="wellDoneMessage";dateCard.classList.add("greenBorder"),messageToCLient(parent,parentClass,message,childClass)}},cvvValue=function(input){switch(input.value){case 0:console.log(3==_fieldValue.length?"good":"wrong"),breaks;case 1:console.log(4==_fieldValue.length?"good":"wrong"),breaks;default:console.log("this isn't correct")}},isEmpty=function(fields){for(var i=0;i<fields.length;i++)fields[i].classList.contains("redBorder")&&fields[i].classList.remove("redBorder"),""==fields[i].value&&fields[i].classList.add("redBorder")},checkingOnSubmit=function(e){e.preventDefault(),isCardNumberValid(),isValidDate(),isEmpty(inputs)},allInputs=document.querySelectorAll("input"),i=0;i<allInputs.length;i++)allInputs[i].addEventListener("paste",pasteString,!1);cardInput.addEventListener("keyup",function(){deleteNaN(this);var _cardType=cardType(this);printCardType(_cardType)},!1),cvvInput.addEventListener("keyup",function(){deleteNaN(cvvInput),cvvValue(this)},!1),el(".btn").addEventListener("click",checkingOnSubmit,!1);
=======
...
>>>>>>> 142982c5e814fa6c6de4f6488b62ca018cf588e1
